...
final statsExpenseFilterProvider = StateProvider<ExpenseFilter>((ref) => ExpenseFilter.variableOnly);
...
final statsDateRangeProvider = Provider<DateRange>((ref) {
  ...
  return switch (period) {
    StatsPeriod.thisWeek => currentWeekRange(),
    StatsPeriod.thisMonth => currentMonthRange(),
    StatsPeriod.last4Weeks => last4WeeksRange(),
...
class CategoryAmount {
  const CategoryAmount({required this.category, required this.amount, required this.percent});
...
final trendProvider = Provider<List<TrendPoint>>((ref) {
  ...
  final all = ref.watch(expenseProvider);
  final filter = ref.watch(statsExpenseFilterProvider);
  final filtered = all.where((e) => filter == ExpenseFilter.all || !e.isFixed);
  ...
  if (period == StatsPeriod.yearly) {
    ...
    return List.generate(5, (i) {
      ...
      final amount = filtered.where((e) => e.date.year == year).fold(0.0, (sum, e) => sum + e.amount);
